...
export class UserMapper {
    constructor(
        @Inject('ENCRYPTION_UTIL') private readonly encryptionService: EncryptionInterface
    ) {}
    ...
    async toEntity(entity: User, dto: Partial<CreateUserDto>): Promise<User> {
        ...
        if (dto.plainPassword && !(await this.isPasswordSame(entity.hashedPassword, dto.plainPassword))) {
            entity.hashedPassword = await this.encryptionService.hash(dto.plainPassword);
        }        

        return entity;
    }

    private async isPasswordSame(hashedPassword: string, plainPassword: string): Promise<boolean> {
